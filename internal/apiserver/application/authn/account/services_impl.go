...
func (s *credentialApplicationService) RotatePassword(
	...
	return s.uow.WithinTx(ctx, func(tx uow.TxRepositories) error {
		// 获取密码凭据
		credential, err := tx.Credentials.GetByAccountIDAndType(ctx, accountID, credDomain.CredPassword)
		if err != nil {
...
func (s *credentialApplicationService) verifyPassword(plainPassword string, credential *credDomain.Credential) bool {
...
func toCredentialResult(cred *credDomain.Credential) *CredentialResult {
	// 使用 Credential 实体的类型判断方法推断凭据类型
	var credType credDomain.CredentialType

	// 优先使用实体的类型判断方法
	switch {
	case cred.IsPasswordType():
		credType = credDomain.CredPassword
	case cred.IsPhoneOTPType():
		credType = credDomain.CredPhoneOTP
	case cred.IsOAuthType():
		// OAuth 类型需要进一步判断具体的 IDP
		if cred.IDP != nil {
			switch *cred.IDP {
			case "wechat":
				credType = credDomain.CredOAuthWxMinip
			case "wecom":
				credType = credDomain.CredOAuthWecom
			default:
				// 未知的 OAuth 类型，使用默认值
				credType = credDomain.CredentialType(*cred.IDP)
			}
		}
	}
