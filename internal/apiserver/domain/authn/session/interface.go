...
// Extender 会话延期器，负责会话延期，并应用会话最大生命周期上限。
...
// RefreshExpirer 刷新过期时间计算器，负责计算下一次 refresh token 的领域过期时间。
