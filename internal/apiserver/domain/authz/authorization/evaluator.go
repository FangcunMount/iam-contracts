...
type EvaluationContext struct {
	EffectiveRoles []meta.ID                            // 主体直接获得的有效角色 ID
...
// Evaluator 匹配资源与动作授权并生成授权决策，不负责持久化或快照管理。
type Evaluator struct{}

// NewEvaluator 创建评估器
func NewEvaluator() Evaluator { return Evaluator{} }

// Evaluate 求值访问请求；未匹配是正常拒绝，候选事实不合法时返回错误。
func (Evaluator) Evaluate(
	...
	return Deny(context.PolicyVersion, evaluatedAt), nil
}
