...
func TestInheritanceRejectsSelfEdge(t *testing.T) {
	_, err := roleinheritance.New(meta.FromUint64(1), meta.FromUint64(1), "operator-1")
	require.True(t, perrors.IsCode(err, code.ErrInvalidArgument))
}

func TestWouldCreateCycleTraversesExistingRoleGraph(t *testing.T) {
	edgeAB, err := roleinheritance.New(meta.FromUint64(1), meta.FromUint64(2), "operator-1")
	require.NoError(t, err)
	edgeBC, err := roleinheritance.New(meta.FromUint64(2), meta.FromUint64(3), "operator-1")
	require.NoError(t, err)
