...
func (r *Runtime) LoadPolicy(ctx context.Context) error {
	...
	if err == nil {
		...
		if err == nil {
			if previous := r.current.Load(); previous != nil {
				if snapshot.version < previous.version {
					err = fmt.Errorf("authorization policy version regressed")
				}
			}
		}
		if err == nil {
		...
		if err == nil {
			...
			loadedPolicyVersion.Set(float64(snapshot.Version()))
...
func (r *Runtime) GetAuthorizationSnapshot(_ context.Context, sub subject.Ref, appName string) (authorizationapp.SubjectSnapshot, error) {
	snapshot := r.current.Load()
	if snapshot == nil || !r.freshSnapshot(snapshot) {
		expiredChecks.Inc()
	...
	return snapshot.SubjectSnapshot(sub, appName)
}

func (r *Runtime) EffectiveRoleNamesForSubject(_ context.Context, sub subject.Ref) ([]string, error) {
	snapshot := r.current.Load()
	if snapshot == nil || !r.freshSnapshot(snapshot) {
		expiredChecks.Inc()
		return nil, perrors.WithCode(code.ErrAuthorizationPolicyUnavailable, "authorization runtime policy unavailable")
	}
	return snapshot.effectiveRoleNamesForSubject(sub)
}
...
func (r *Runtime) RecordPolicyVersionEvent(version int64, eventAt time.Time) {
	r.health.recordEvent(version, eventAt)
...
func (h *runtimeHealth) recordEvent(version int64, at time.Time) {
	h.mu.Lock()
	defer h.mu.Unlock()
	h.target = max(h.target, version)
...
func (h *runtimeHealth) versionLag(snapshot *Snapshot) int64 {
	...
	return max(0, h.target-snapshot.version)
}
...
func (h *runtimeHealth) details(now time.Time, snapshot *Snapshot) map[string]any {
	...
	details := map[string]any{
		...
		"last_event_version": h.lastEventVersion,
	...
	if snapshot != nil {
		details["loaded_policy_version"] = snapshot.Version()
	}
	return details
}
