...
type ChildPO struct {
	base.AuditFields
	Name     string `gorm:"column:name;type:varchar(64);not null;comment:儿童姓名"`
	IDCard   string `gorm:"column:id_card;type:varchar(20);uniqueIndex;comment:身份证号码"`
	Gender   uint8  `gorm:"column:gender;type:tinyint;not null;default:0;comment:性别"`
	Birthday string `gorm:"column:birthday;type:varchar(10);comment:出生日期"`
