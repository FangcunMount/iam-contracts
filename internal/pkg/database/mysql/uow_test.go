...
func TestUnitOfWorkNestedRequiredReusesOuterTransaction(t *testing.T) {
	...
	var hooks []string
	err := uow.WithinTransaction(context.Background(), func(txCtx context.Context) error {
		outerTx, err := dbmysql.RequireTx(txCtx)
		require.NoError(t, err)
		require.NoError(t, dbmysql.AfterCommit(txCtx, func(context.Context) error {
			hooks = append(hooks, "outer")
			return nil
		}))

		return uow.WithinTransaction(txCtx, func(nestedCtx context.Context) error {
			nestedTx, err := dbmysql.RequireTx(nestedCtx)
			require.NoError(t, err)
			require.Same(t, outerTx, nestedTx)
			require.NoError(t, dbmysql.AfterCommit(nestedCtx, func(context.Context) error {
...
func TestAfterCommitRequiresTransaction(t *testing.T) {
	err := dbmysql.AfterCommit(context.Background(), func(context.Context) error { return nil })
	require.ErrorIs(t, err, dbmysql.ErrActiveTransactionRequired)
}
