...
| REST v3 契约 | `api/rest/authz.v3.yaml` |
...
| 语义收敛工具 | `internal/apiserver/maintenance/authz_v3_converge.go`、`cmd/iam-maintenance` |
...
| 修改角色图 | 同时保护直接/有效角色、Tenant 隔离、多层/菱形继承、32 层边界与 Decision 等价性 |
...
| MySQL Replace 并发 | `replace_managed_assignments_mysql_concurrent_test.go`（需 `MYSQL_HOST`） | 独立租户 fixture + 事务内 Role-read barrier + 最终集合/精确版本/Outbox 断言 | CI 默认跳过；不能替代全部生产并发场景 |
...
1. 确认 JWT 解析后 UserID/TenantDomain。
2. 从 router 找到精确 Resource/Action，不要猜角色名。
3. 分别在当前 Tenant 和 platform 查 Subject effective roles/Grant。
...
1. 固化 Subject→Role、Role→ParentRole、Tenant domain、direct/effective roles、多层继承和循环失败的表征测试。
...
| 数据审计、事务收敛和证据 | `maintenance/authz_hardening.go`、`cmd/iam-maintenance/authz_hardening.go` |
