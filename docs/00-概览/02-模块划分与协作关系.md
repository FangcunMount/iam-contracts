...
| 能访问什么 | AuthZ | Subject、Resource、Action、ObjectAttribute、Role、Assignment、DirectRoles、PermissionGrant、ConstraintSet |
...
```text
维护 Resource、Action、Role、Assignment、PermissionGrant 和 ConstraintSet；运行时仅解析直接 Assignment，多角色权限取并集；
执行权限检查；
管理授权写入；
通过 PolicyVersion 和 Outbox 传播授权事实变化；
从 MySQL 事实编译原生不可变快照；直接角色索引装载 Assignment，不再计算继承闭包。
```
...
| 把历史 Casbin 实现当成当前业务模型 | 权限匹配由领域 Evaluator 执行；运行时只解析直接 Assignment |
...
| AuthZ | Subject、Resource、Action、ObjectAttribute、Role、Assignment、DirectRoles、PermissionGrant、ConstraintSet、AuthorizationDecision、PolicyVersion | Identity.User/Profile 引用；AuthN 认证结果 | 登录认证、Token 签发、User/Profile 写模型和业务关系事实 |
