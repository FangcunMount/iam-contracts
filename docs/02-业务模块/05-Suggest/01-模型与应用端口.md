...
`Principal.OrgID` 和 `OrgIDs` 表示业务组织与数据可见范围；主体资格由 UserID 表达，授权管理保护由 Role.ManagementProtection 表达。
...
| 条件 | 生成的 Scope |
| --- | --- |
| `PlatformListAllowed=true` | `allProfiles=true`；手机号能力取 `AllProfilesMobileSearchAllowed` |
| 否则 | operator + principal orgs + visibility ProfileIDs；手机号能力取 `ScopedMobileSearchAllowed` |
