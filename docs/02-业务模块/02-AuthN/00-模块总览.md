...
| 确认身份与准入 | 如何确认现在是你，而且当前仍允许认证 | `Challenge`、`IdentityProof`、`AuthDecision`、`Principal`、`AdmissionDecision` | `Authenticator`、`AuthStrategy`、`AdmissionPolicy` |
| 延续认证状态 | 如何让系统持续相信是你，并能续期或终止 | `登录结果`、`Session`、`AccessToken`、`RefreshToken` | `SignIn`、`Verifier`、`Refresher`、`Revoker`、`LifetimePolicy` |

AuthN 局部对象关系见 [领域模型](01-领域模型与认证策略.md)。跨模块聚合边界使用 [V8 核心模型图](../../_images/architecture/core-domain-model-v8.png)，领域模型与服务综合讲解使用 [核心聚合图](../../_images/architecture/core-domain-model-v8.png)（[SVG 图源](../../_images/architecture/core-domain-model-v8.svg)）。整体图适合放大查阅，正文先沿三个子域阅读。
...
4. 如何把一次成功的身份核验延续成可撤销、可刷新、可跨服务验证的登录状态？

它们在一次用户登录中的协作主线是：

```text
建立认证关系：LoginIdentity + Credential
  -> 身份核验：IdentityProof / Challenge -> AuthDecision -> Principal
  -> 登录准入：AdmissionPolicy -> admission.Decision
  -> 会话建立：SessionCreator -> Session
  -> 令牌颁发：InitialTokenIssuer -> TokenPair
  -> 登录成功：Result = Principal + TokenPair
  -> 后续验证 / 刷新 / 撤销
```
...
- Signing key metadata：签名密钥的 active/grace/retired 等生命周期事实。

### 2.2 短期事实

- `Challenge`：OTP/OAuth state 的一次性交互状态；
- `Principal`：本次身份核验成功后的运行时结果，权威认证上下文为 `AuthenticationContext{Method, Realm, AMR, AuthenticatedAt}`；
- `Session`：一段登录期的在线状态，保存认证上下文与允许续期投影的权威事实；
- `登录结果`：应用层 Result，包含 Principal 与 TokenPair；Session 已由独立创建步骤保存；
...
| --- | --- | --- | --- |
| username | 固定 `default` | username/email/phone fallback | 空 |
| phone | `global` | phone | 空 |
| wechat_minip | appID | openid | unionid（可选） |
| wechat_open | appID | openid | unionid（可选） |
| wecom | corpID | 企业微信 userid | 空 |

`realm` 不是装饰字段。openid 只在一个 appID 内稳定；如果去掉 realm，不同外部应用会错误合并身份。用户名统一使用 default。
...
MethodSelector
  ...
  -> CredentialRecorder
  -> AuthDecision
  -> 登录应用准备独立 TokenContext
  -> AdmissionPolicy.Evaluate → Decision
  -> SessionCreator.Create
  -> InitialTokenIssuer.IssueInitialTokens
       -> TokenSetMinter.MintTokenSet
       -> RefreshTokenSaver.SaveRefreshToken
```

- MethodSelector 确认公开允许的登录方法与 payload 形状；
- ProofFactory 构造领域 credential；外部 OAuth 路径完成 code/state 交互，手机号路径只构造输入，OTP 消费在身份核验策略调用 verifier 时发生；
...
- InitialTokenIssuer 接收已创建的 Session，调用 TokenSetMinter 并保存初始 RefreshToken。
- Token 门面返回 TokenPair；Session 的创建与失败补偿由 SignIn 负责。
...
| SignIn / InitialTokenIssuer | Admission 后创建 Session，再 mint/save refresh | mint 或 save refresh 失败会补偿撤销 Session；补偿失败仍可能留下孤儿 Session |
...
| Signing-key mutation | key 元数据先提交，publish cache 后刷新 | post-commit refresh 失败时事实已变但缓存可能滞后 |
