...
应用命令为 `LinkRequest{UserID, AuthenticatedAt, Input}`；Input 是上述四种变体。`LinkResult` 返回 Identity 与 Reused，表示新建或复用。公开 DTO 由 transport 映射，字段契约见 `api/rest/authn.v3.yaml` 与 `api/grpc/iam/authn/v3/authn.proto`。
