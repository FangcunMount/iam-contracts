...
`Principal` 只包含 `UserID`、`LoginIdentityID` 和 `AuthenticationContext{Method, Realm, AMR, AuthenticatedAt}`，表达身份核验确认的主体和核验事实。会话和令牌不属于该对象；AuthN 不再提供请求租户选择。
...
`Principal` 是认证结果；`AuthenticationContext` 是认证事实快照；独立的 `TokenContext` 保存令牌业务域和组织上下文。`Realm` 是登录身份命名空间，AuthN 已移除数值租户字段，保留字符串 `TenantDomain` 授权域；JWT 的既有 `tenant_id` 字段映射后者。
...
登录结果不再包含数值租户字段。身份核验成功日志不再输出未经该阶段确认的 tenant_id。上述结构调整不新增租户成员准入规则，也不改变 RefreshToken 历史快照读取、轮换规则或登录失败补偿。
...
旧登录负载中多余的数字 `tenant_id` 按未知 JSON 字段忽略，不能选择账号命名空间；新协议说明和 SDK 不再提供该参数。REST 验证结果删除历史数字字段，字符串 `tenant_domain` 保留。JWT/gRPC 的字符串 `tenant_id` 表示授权域，继续保持既有契约。

Redis 新写入不包含 Session 的 `TenantID` 或 RefreshToken 顶层 `tenant_id`；旧记录可读取，其数字字段被忽略，不影响身份、期限和刷新轮换。历史认证快照仍按原规则兼容。
