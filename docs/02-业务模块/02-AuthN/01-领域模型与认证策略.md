...
Session 记录 UserID、LoginIdentityID、AuthContext、BusinessContext 和创建/过期/撤销信息，状态为 `active`、`revoked` 或 `expired`。它让系统具备以下能力：
...
`Principal` 是身份核验结果；`AuthenticationContext` 是身份核验事实快照；`BusinessContext` 保存会话业务组织和附加属性快照，供 Token 签发与刷新读取；当前新登录不填充，仅延续历史会话已有数据。`Realm` 是身份提供方命名空间。
...
AuthN 不提供多租户登录能力。登录请求、SDK PasswordPayload、Proof、Session、AccessToken、RefreshToken 和登录结果均不包含数字 TenantID；仅包含 BusinessContext 的会话创建包装也已删除。用户名注册与查找固定使用 `default` Realm，微信 AppID、企业微信 CorpID 等外部身份命名空间保持原义。
