...
| `登录结果` | 单次颁发结果 | 认证成功后建立了哪个 Session 并交付哪组令牌 | 长期持久化、资源授权 |
...
本图选择持久事实的关键字段，ProviderKey、MaterialHash、TokenMetadata 为语义分组，不是数据库列清单。请求级 AuthCredential、AuthDecision、Principal、AdmissionDecision 和 登录结果 不独立持久化，沿下文策略与颁发链路解释。Access/Service Token 是已颁发凭证，签名密钥有独立生命周期；完整聚合分类见 [V8 图](../../_images/architecture/core-domain-model-v8.png)，避免在局部图同时展开所有运行时对象。
...
`VerifiedTokenClaims` 和 `ConsumedRefreshToken` 表达不同令牌事实；`TokenSetMinter`、`Refresher`、`Verifier`、
...
旧的 AuthenticationGrant / SessionEstablishmentResult 结果容器已移除。应用层 Result 组合 Principal 与 TokenPair，不是 OAuth authorization grant，也不是持久化聚合。SignIn 协调准入、会话创建、令牌签发和失败补偿；这些步骤不是全局原子事务。

`Principal` 是认证结果；`AuthenticationContext` 是认证事实快照；独立的 `TokenContext` 保存令牌业务域和组织上下文。`Realm` 是登录身份命名空间，数值 `TenantID` 与字符串 `TenantDomain` 不同；JWT 的既有 `tenant_id` 字段映射后者。

`AccessTokenIssueContext`（旧名 `AccessTokenSubject`）是编码器的签发输入，既不是单个 JWT `sub`，也不是完整 Claims Set。`VerifiedTokenClaims` 表达验签及声明校验阶段的结果，不等于完成会话准入、目标受众和资源授权检查。
...
历史 V7/V8 图片保留 AuthenticationGrant、GrantIssuer 标签；当前结果容器已移除，相关职责由应用层 SignIn 与 InitialTokenIssuer 分担，名称以本文及代码为准。
