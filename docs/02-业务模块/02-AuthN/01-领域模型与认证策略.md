...
`Principal` 只包含 `UserID`、`LoginIdentityID` 和 `AuthenticationContext{Method, Realm, AMR, AuthenticatedAt}`，表达身份核验确认的主体和核验事实。请求选择的租户、会话和令牌不属于该对象。
...
Session 记录 UserID、LoginIdentityID、历史请求租户值、AuthContext、TokenContext 和创建/过期/撤销信息，状态为 `active`、`revoked` 或 `expired`。它让系统具备以下能力：
...
| `PasswordProof.RealmTenantID` | 仅用于选择 username 身份命名空间，不证明租户成员关系。其他方式不携带该字段。 |
| `WecomProof.ProviderUserID` | 企业微信的用户标识，区别于 IAM 的 `Principal.UserID`。 |
| `Principal` | 仅保存核验确认的用户、登录身份和认证上下文，不承载请求租户。 |
| `AuthDecision` | 成功身份只从 Principal 读取；RejectedLoginIdentityID 仅定位失败身份。CredentialUpdate 聚合长期凭据记录意图，Rotation 仅能随成功记录且必须有材料。 |
| `session.CreationContext` | 应用层提供 RequestedTenantID 与 TokenContext。RequestedTenantID 是历史请求上下文，不表示租户归属已经通过检查。 |
| `Session.TenantID` | 保留历史数值请求字段以兼容会话与令牌存储；与 JWT 使用的字符串 TenantDomain 分开。 |
...
登录结果从完成建立的 Session 读取历史租户值。身份核验成功日志不再输出未经该阶段确认的 tenant_id。上述结构调整不新增租户成员准入规则，也不改变 RefreshToken 历史快照读取、轮换规则或登录失败补偿。
