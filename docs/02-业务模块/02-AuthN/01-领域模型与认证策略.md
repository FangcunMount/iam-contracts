...
`AccessTokenIssueContext` 是领域层从 Session 提取的签发上下文；编码器输入是组装完成的 `AccessTokenClaims`，既不是单个 JWT `sub`，也不是完整 Claims Set。`AccessTokenClaims` 表达声明事实，不承诺经过验签，不等于完成会话准入、目标受众和资源授权检查。
