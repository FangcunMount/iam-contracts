...
### 为什么身份核验成功还要 AdmissionPolicy

正确密码只证明秘密匹配，不代表被封禁 User 或已禁用 LoginIdentity 仍可进入系统。`AdmissionPolicy` 同时读取两个状态，分别映射 blocked、disabled、inactive 等结果。
它由登录应用 `SignIn` 以及领域 `Refresher`、`Verifier` 复用，判断是否允许建立或维持认证状态，不负责 AuthZ 资源授权。SignIn 显式消费 Decision，
避免将这个不变量遗落在其他颁发入口。
...
`admission.Policy.Evaluate` 返回 `(Decision, error)`。Decision 包含 Subject、Outcome 和 Reason：业务通过与拒绝都属于正常判定；error 表示依赖读取失败等导致无法完成评估。登录应用只在 `IsAdmitted()` 为真时继续创建 Session，并在应用边界把拒绝原因映射为既有错误码。刷新和在线验证仍可使用 `Require` 作为强制检查适配。Decision 不包含 SessionID 或 Token，也不作为可跨请求复用的通行凭证。
