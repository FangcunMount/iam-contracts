...
快照和确认时间作为同一原子值发布，防止读到旧快照的请求误用新快照的证明。快照构建深复制 Grant、Resource、ConstraintSet、schema、嵌套集合和标量指针；查询结果和版本映射也由调用方独立持有。
...
1. 加载可信属性配置，再加载初始快照；配置文件错误、非法既有条件 Grant 或非法继承图阻止启动。
