...
AuthZ 管理“谁拥有什么角色，以及这些角色可以执行什么动作”。它不负责认证身份、不加载业务对象，也不替业务模块判断对象归属。
...
- 进程内授权：IAM 自身管理路由使用 `RequirePermission` 检查明确的 Resource/Action。
...
Subject 解决“谁”，Resource 和 Action 解决“要做什么”，ObjectAttributes 只解决“这个具体对象是否满足条件”。对象属性不能反过来修改主体或角色。
...
Bearer user JWT
  ...
  -> AuthZ Middleware.RequirePermission(resource, action)
...
方法 ACL 只回答“服务能否调用此 RPC”；request constraints 继续回答“能改哪类 Subject、哪些 Role”。二者缺一都会把接口级授权误当成数据级授权。
...
| Assignment | AuthZ 直接授权 | Subject、Role、active 唯一 | 角色图入口 |
