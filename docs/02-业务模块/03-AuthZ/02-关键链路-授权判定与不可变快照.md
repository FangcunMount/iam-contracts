...
AuthZ v4 在 reload 时从 MySQL 构建完整不可变快照，并以原子替换方式发布。请求期不查权限事实表：直接角色索引解析主体角色，
...
`MySQLSource.Load` 读取五类数据：Role、未删除 Assignment、未撤销 PermissionGrant、
Resource 与全局 PolicyVersion。继承表退役后不再装载。
...
- 缺少 ID、管理保护属性或 name 的 Role；
- 重复 Role name 或重复 Role ID；
- 无效、重复 Resource key 或重复 Resource ID；
- Assignment 引用未知 Role；
- PermissionGrant 引用未知 Role、未知 Resource 或违反 catalog/schema；
- 无法解析的 Subject key、ConstraintSet 或资源模式。

加载、校验、构建与发布通过同一个可取消入口串行执行。候选全局版本不得低于已发布版本。只有所有数据都通过，新 Snapshot 才会进入 `atomic.Pointer`。任意一条错误都会让整次 reload 失败，而不是跳过脏数据继续发布。
...
- 策略版本；
- Assignment 构建的直接角色索引；
- RoleID 下的 PermissionGrant 候选集合；
...
`grantsByRole` 是按 RoleID 缩小候选集的索引，不是把所有授权维度固定为物理四维数组。Resource/Action/Constraint 仍在候选 Grant 内依次匹配。
...
sequenceDiagram
  ...
  Caller->>GRPC: Subject, Resource, Action, trusted attrs
...
1. 校验 Subject、Resource、Action 和属性输入。
2. 从直接角色索引得到当前 Subject 的角色集合。
3. 按直接角色收集候选 Grant。
...
`Request` 必须包含非空 Subject、四段具体 Resource key 和具体 Action。请求中的 Resource 不允许通配；通配只属于 Grant Pattern。
...
不可变角色图使用 `subject.Ref` 与 RoleID 组成的类型化键，先取 Subject 的直接角色，再求全部可达 parent role，最后稳定排序。
...
- `roles`：与 `direct_roles` 相同，均为应用范围内的直接 Assignment；
...
新数据集如果含有成环继承、非法角色引用或不符合 schema 的 Grant，将其“部分发布”会形成无法对应任何数据库版本的策略。当前选择是：

```text
新快照完整成功且全局版本不回退 -> atomic swap(snapshot + proof)
...
当前每个 ConstraintSet 最多 8 个 predicate。它们是可控上界，但不等于经过任意规模生产压测。若 Grant 数量大幅增长，应使用实际角色/ Grant 分布重新基准测试，
而不是仅从无数据库查询推导为恒定成本。
...
| `iam_authz_native_loaded_policy_version_max` | 当前快照最大版本 | 所有实例都已加载目标版本 |
| `iam_authz_native_policy_version_lag_max` | 已观察事件与已加载版本滞后 | 没收到的事件不存在 |

指标不带用户、角色或动态受众等高基数标签，运行时健康详情另外返回最后事件版本、已加载全局版本、reload 时间和同步 channel。生产收敛应结合实例维度健康数据，不应只看一个全局 gauge。
...
Snapshot 深复制输入的 Grant、Resource、条件集合、schema、嵌套切片及标量指针。内部 `evaluationContext` 只供 Runtime 调用，不向外暴露候选 map 和实体指针；主体快照、直接角色和版本映射返回独立副本。
