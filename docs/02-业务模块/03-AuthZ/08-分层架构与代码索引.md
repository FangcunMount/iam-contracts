...
| 语义收敛工具 | `internal/apiserver/maintenance/rolemodel`、`cmd/iam-maintenance`（`role-model-migrate`） |
...
| 修改直接角色分配 | Assignment 写入、snapshot/direct roles、多角色并集、测试 |
...
| 修改角色解析 | 同时保护直接角色、角色管理保护与 Decision 等价性；继承能力已退役 |
...
| 共用直接角色索引 | `infra/authz/runtime/direct_roles.go`、`snapshot.go` |
...
| 数据审计、事务收敛和证据 | `maintenance/authz_hardening.go`、`cmd/iam-maintenance/role_model.go` |
