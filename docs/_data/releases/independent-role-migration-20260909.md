...
状态：兼容实现与验证中；未执行生产 apply，未退役生产继承表。
...
5. 完成回滚演练与调用方核对，再发布不依赖继承表的服务，归档历史结构、数据、校验和及执行清单后用新增结构迁移删除表。

Rollback 只能补偿未被后续授权写入修改的完整事实；冲突时停止。PolicyVersion 必须继续递增。应用查询边界与角色发放成组回滚。删表后须先恢复结构及归档数据，再恢复旧二进制。

## 已验证与未完成

IAM 与 QS 全量 Go 测试通过；迁移测试在 SQLite 和本机真实 MySQL 9.3 通过（尚不等于生产 MySQL 版本验收）。运营端 80 个测试套件、260 个测试通过。生产构建继续复核。

最终继承代码删除、结构迁移完整链、生产版本部署、数据 apply、运行时收敛与真实业务验收尚未完成。
