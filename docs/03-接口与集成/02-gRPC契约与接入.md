...
- 当前已注册的 gRPC 服务包括 `AuthService`、`JWKSService`、`IdentityRead`、`GuardianshipQuery`、`GuardianshipCommand`、`IdentityLifecycle`、`IDPService`。
...
- 当前没有发现 `Authz` 的 gRPC 注册点，因此不应把授权能力写成“当前对外 gRPC 已开放”。
- 某个 `proto` 中存在的方法，如果当前未被 `Register...Server(...)` 注册，只能写成合同能力或未来能力，不能写成当前运行面。
