...
│               └── [reserved]
│
├── application/authn/              # 应用层 - 应用服务和用例编排
│   ├── account/                    # 账户管理应用服务
...
│       └── uow.go                  # 事务管理
│
├── infra/                        # 基础设施层 - 外部依赖实现（按技术栈划分）
│   ├── mysql/                    # MySQL 仓储实现
│   │   ├── account/              # 账户仓储（聚合）
│   │   └── jwks/                 # JWKS 仓储
│   ├── redis/                    # Redis 适配器
│   │   ├── token/                # Token 存储
│   │   ├── accesstoken_cache.go  # 微信 SDK Token 缓存
│   │   └── otp_verifier.go       # OTP 校验器
│   ├── crypto/                   # 密钥生成与存储、密码哈希
│   ├── jwt/                      # JWT 生成器与签名
│   ├── authentication/           # TokenVerifier 适配器
│   ├── scheduler/                # 密钥轮换调度器
│   ├── wechat/                   # 微信认证适配器
│   └── wechatapi/                # 微信开放平台 SDK 封装
│
└── interface/authn/               # 接口层 - 对外暴露 API
    └── restful/                    # RESTful API
        ├── router.go               # 路由注册
        ├── handler/                # HTTP 处理器
        │   ├── base.go             # 基础处理器
