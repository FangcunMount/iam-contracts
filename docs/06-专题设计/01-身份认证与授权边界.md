...
AuthZ 把 Principal 映射为 Subject，再用当前原生不可变快照对 Resource/Action 和受信 ObjectAttributes 判定。
...
- 权限还受 Tenant、Role、PermissionGrant、ConstraintSet、受信对象属性和当前状态约束；
...
身份核验的 Principal 不携带 TenantID；请求租户通过会话创建上下文单独传递，不能当作已核验的租户归属。AuthZ 的 Assignment、RoleInheritance、PermissionGrant 和运行时角色图都按 tenant/domain 隔离；
但 tenant 必须由可信上下文确定。若客户端可随意选 tenant，即使 Grant 求值完全正确也可能在错误域中做判定。

认证建立“谁”，tenant resolution 建立“在哪个授权域”，AuthZ 再回答“能做什么”。三步都属于安全边界。
...
| authorization power | 在当前上下文能执行什么 | Subject、Assignment、RoleInheritance、PermissionGrant、Resource、Action、ObjectAttributes | grant、revoke、策略 reload |
...
ProfileLink(User, Profile, relation)
    ...
    ↓ 可作为可信事实输入
业务服务加载对象并验证关系
    构造受信 ObjectAttributes
    ↓
Check(Subject, Tenant, Resource, Action, ObjectAttributes)
    由 Assignment + RoleInheritance + PermissionGrant + ConstraintSet 得到 allow/deny
```
...
“关系即权限”只在规则极简单且永不演进时看似方便。一旦动作、租户、例外和审计出现，隐式权限就会变得不可解释。
...
| 当前 Assignment/RoleInheritance/PermissionGrant | 不检查 | 不负责资源授权 | 检查当前 immutable runtime snapshot |
...
工程上可以串联，语义上必须保留阶段：先建立可信 Principal，再构造 Subject/Tenant/Resource/Action/ObjectAttributes，最后 Check。
...
3. Tenant 和 ObjectAttributes 必须由服务端可信上下文建立；
...
不是。所有权只是授权输入，还需按 Action、Tenant、PermissionGrant、ConstraintSet 和业务状态判定。尤其导出、分享、删除等动作通常需要额外权限。
