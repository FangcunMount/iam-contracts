...
- 权限还受 Role、PermissionGrant、ConstraintSet、受信对象属性和当前状态约束；
...
3. Subject 和 ObjectAttributes 必须由服务端可信上下文建立；
...
不是。所有权只是授权输入，还需按 Action、PermissionGrant、ConstraintSet 和业务状态判定。尤其导出、分享、删除等动作通常需要额外权限。
