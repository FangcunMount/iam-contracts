...
- `02-业务域` 现在只承载模块设计正文，不再承担长链路专题或接入边界说明。
- 当前 4 篇正文已经统一到同一骨架：`30 秒了解系统 -> 模块边界 -> 运行时示意图 -> 模型与服务 -> 核心设计 -> 边界与注意事项 -> 代码锚点索引`。
- `authn / authz / 用户域（代码目录 `uc`）` 是主业务域；`suggest` 是依附用户域的补充读侧能力。
- 这一层最重要的是讲清“模块本身是什么”，不是把跨层时序和合同漂移再讲一遍。
...
- 图保留，但不机械地堆在文首。
- 第一张图优先回答“模块长什么样”，通常放在“运行时示意图”或“模型关系图”。
- ER 图和分层图分别回答“数据关系”和“依赖方向”，不要让一张图承载两种视角。
- `### 核心...` 小节里的图只在它第一次解释那个问题时出现，不重复摆同类总图。
- 如果某篇没有合适的 ER，例如 `suggest`，应明确写 **N/A**，而不是硬补一张没有信息增量的图。
