...
func TestMetricsUnaryInterceptorRecordsMethodAndCode(t *testing.T) {
	...
	err := interceptor(context.Background(), "/iam.authz.v4.AuthorizationService/Check", nil, nil, nil, func(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, opts ...grpc.CallOption) error {
		return status.Error(codes.PermissionDenied, "denied")
	})
	require.Error(t, err)
	require.Equal(t, 1, collector.calls)
	require.Equal(t, "/iam.authz.v4.AuthorizationService/Check", collector.method)
...
func TestCircuitBreakerInterceptorOpensAfterFailureThreshold(t *testing.T) {
	...
	err := interceptor(context.Background(), "/iam.authn.v3.AuthService/VerifyToken", nil, nil, nil, func(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, opts ...grpc.CallOption) error {
